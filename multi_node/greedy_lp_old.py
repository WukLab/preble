...
class LPGurobiGreedyTraversal:
    ...
    def traverse_and_optimize(
        ...
        for prefix_node in modified_nodes:
            ...
            for gpu_index, var in enumerate(lp_node.variables):
                ...
                per_gpu_recomp_cost[gpu_index] += recomp_cost
        ...
        for gpu_index in range(self.num_gpus):
            ...
            self.model.addConstr(
                per_gpu_mem_load_cost[gpu_index] + per_gpu_load_cost[gpu_index] + per_gpu_recomp_cost[gpu_index]
                <= max_per_gpu_cost,
                name=f"max_per_gpu_cost_constr_{gpu_index}",
            )
            # updated load cost
        # Set objective
        self.model.setObjective(max_per_gpu_cost, GRB.MINIMIZE)
    ...
    def insert_into_cache_and_solve(
        self,
        ...
        self.traverse_and_optimize(node, modified_nodes=modified_nodes, split_nodes=split_nodes)
        return node

class GurobiGreedyLPSchedulerV1:
