...
def profile_matching_throughput(rps: float, t: int, model_details: ModelDetails):
    ...
    prompts = [
        (get_react_workload(f"Workload {i} "), sampling_params)
...
if __name__ == "__main__":
    ...
    logging.debug(
        f"Starting Experiment at {datetime.datetime.now(datetime.timezone.utc)}"
    )
    model_name = "mistralai/Mistral-7B-v0.1"
    available_gpus = [0]
    loader = MultiNodeLoader(available_cuda_nodes=available_gpus)
    model_details = loader.load_model(model_name, gpus=available_gpus, urls=[])
