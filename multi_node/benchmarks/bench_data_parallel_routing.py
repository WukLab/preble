...
def test_oracle_random_basic(num_workloads, distribution_of_non_shared, num_requests, rps=0.0, model_name="mistralai/Mistral-7B-v0.1"):
    ...
    logging.debug(
        f"=====STARTING BENCHMARK OF {num_workloads} WORKLOADS, {distribution_of_non_shared} NON-SHARED, {num_requests} REQUESTS====="
    )
    ...
    def load_and_run_benchmark(policy):
        ...
        if policy == DataParallelRuntimeSelectionPolicy.CUSTOM:
            model_details.update_runtime_selection_policy(
                ...
                custom_runtime_selector=Oracle(num_nodes=2),
            )
        else:
        ...
        logging.debug(
            f"Params=({model_name}, {num_workloads}, {distribution_of_non_shared}, {num_requests}, {policy}) Overall Latency: {latency}"
        )
        logging.debug(
            f"Params=({model_name}, {num_workloads}, {distribution_of_non_shared}, {num_requests}, {policy}) Overall Throughput: {num_requests / latency}"
        )
        logging.debug(
