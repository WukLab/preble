...
class RadixCache:
    ...
    def _split_node(self, key, child: TreeNode, split_len, node_map, depth_limit, current_depth):
        # new_node -> child
        new_node = TreeNode()
    ...
    def _insert_helper(self, node: TreeNode, key, value, node_map, modified_nodes, depth_limit, current_depth, split_nodes):
        node.last_access_time = time.time()
        node.ref_counter += 1
        for c_key, child in node.children.items():
            prefix_len = match(c_key, key)
            ...
            if prefix_len == len(c_key):
                if prefix_len == len(key):
                    ...
                    return self._insert_helper(child, key, value, node_map=node_map, modified_nodes=modified_nodes, depth_limit=depth_limit, current_depth=current_depth + 1, split_nodes=split_nodes)
...
class LPGurobiGreedyTraversal:
    ...
    def traverse_and_optimize(self, 
        ...
        for gpu in range(self.num_gpus):
            lp_node.variables[gpu] = self.model.addVar(vtype=GRB.BINARY, name=f"x_{gpu}")

        self.model.update()
        
        self.model.addConstr(gp.quicksum(lp_node.variables) >= 1) # at least 1 variable should be one
        ...
        for gpu_index in range(self.num_gpus):
            ...
            per_gpu_load_cost[gpu_index] += lp_node.variables[gpu_index] * total_decode_time
        ...
        for gpu_index in range(self.num_gpus):
            ...
            self.model.addConstr(per_gpu_mem_load_cost[gpu_index] + per_gpu_load_cost[gpu_index]<= max_per_gpu_cost, name=f"max_per_gpu_cost_constr_{gpu_index}")
            # updated load cost
        # Set objective
        ...
        for gpu in selected_gpus:
            self.current_load_cost[gpu] += total_decode_time # Increase by decoding time to each gpu
...
if __name__ == "__main__":
    ...
    # for i in range(3):
    #     texts += [f"Workload 1 ABCD " + lorem_ipsum]
    ...
    #     split_nodes = {}
    #     leaf_node = cache.insert(tuple(input_ids[i]), node_map=node_map, all_modified_nodes=modified_nodes, depth_limit=lp_tree_traversal.depth_limit, split_nodes=split_nodes)
    ...
    #     #         node_map[new_node_id] = node_map[node_id]

    #     runtime = lp_tree_traversal.traverse_and_optimize(leaf_node, previous_cost={}, modified_nodes=modified_nodes)
    #     modified_nodes = set()
